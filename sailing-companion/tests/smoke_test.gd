...
func _process(_delta: float) -> void:
	...
	if _frames == 5:
		...
		_check("Scene3D has sail rig, wheel, captain", _main.get_node("Scene3D/SailRig") != null and _main.get_node("Scene3D/Wheel3D") != null and _main.get_node("Scene3D/Captain3D") != null)
	...
	if _frames == 30:
		...
		var cap3d := _main.get_node("Scene3D/Captain3D")
		_check("Captain3D placed in front of camera", cap3d.position.z < 0.0)
		print("  boat pos=%s rot=%.4f  captain3d=%s bob=%.3f" % [boat.position, boat.rotation, cap3d.position, boat.bob_normalized])
	...
	if _frames == 530:
		...
		_check("Sail rig boom rotates with sail angle", is_finite(rig._boom_pivot.rotation.y))
		print("  boom yaw=%.2f heel_visual=%.2f hud compass size=%s" % [rig._boom_pivot.rotation.y, _main.get_node("Boat").heel_visual, _main.get_node("UI/Hud/CompassStrip").size])
